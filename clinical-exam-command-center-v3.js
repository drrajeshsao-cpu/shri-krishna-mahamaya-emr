/* Mahamaya Clinic EMR V5.55 — Clinical Examination Scientist Command Center v3.
   UI hierarchy/navigation enhancement only. It does not change clinical schemas, login or Firebase. */
(function(){
'use strict';
const VERSION='V5.55',BUILD='2026.08.15-R32',UI='MCE-CC3';
const $=id=>document.getElementById(id);
const qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
const section=()=>document.getElementById('examSection');
const ws=()=>section()?.querySelector('.gei-workspace');
const state={tools:false,active:'Core'};
...
function setActiveLabel(btn){
 ...
 state.active=tabLabel(btn); badge.textContent=state.active;
}
...
function consolidateConsole(){
 ...
 qa('.gei-tabbtn',tabbar).forEach((b,i)=>{
   b.title=`Open ${tabLabel(b)}${i<8?` • Alt+${i+1}`:''}`;
...
function enhanceMoreSigns(){
 ...
 Object.entries(map).forEach(([key,id])=>{
   ...
   let badge=btn.querySelector('.gei-cc-substate');if(!badge){badge=document.createElement('span');badge.className='gei-cc-substate';btn.appendChild(badge)}
   const [cls,label]=stateClass($(id)?.value);btn.classList.remove('cc-positive','cc-negative','cc-partial');if(cls)btn.classList.add(cls);if(badge.textContent!==label)badge.textContent=label;
 });
}
...
function bindLive(){
 document.addEventListener('change',e=>{if(e.target.closest('#examSection'))setTimeout(enhanceMoreSigns,0)});
 document.addEventListener('input',e=>{if(e.target.closest('#examSection'))setTimeout(enhanceMoreSigns,70)});
 document.addEventListener('keydown',keyboardNav);
 const obs=new MutationObserver(()=>{if(consolidateConsole())enhanceMoreSigns()});
 const w=ws();if(w)obs.observe(w,{childList:true,subtree:true});
}
function wrapShowExam(){
 if(typeof window.showExam!=='function'||window.showExam.__cc3Wrapped)return;
 const old=window.showExam;window.showExam=function(){const r=old.apply(this,arguments);setTimeout(()=>{ensureHeader();consolidateConsole();enhanceMoreSigns()},40);return r};window.showExam.__cc3Wrapped=true;
}
function init(){ensureHeader();consolidateConsole();enhanceMoreSigns();bindLive();wrapShowExam();setTimeout(()=>{ensureHeader();consolidateConsole();enhanceMoreSigns();wrapShowExam()},450)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(init,30));else setTimeout(init,30);
window.MAHAMAYA_EXAM_COMMAND_CENTER={version:VERSION,build:BUILD,ui:UI,refresh:()=>{ensureHeader();consolidateConsole();enhanceMoreSigns()}};
})();
